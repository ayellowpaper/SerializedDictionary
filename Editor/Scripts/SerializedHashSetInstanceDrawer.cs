...
{
    public class SerializedHashSetInstanceDrawer
    {
        ...
        private HashSetListState _activeState;
        ...
        internal HashSetSearchListState SearchState { get; private set; }
        internal HashSetDefaultListState DefaultState { get; private set; }
        ...
        public SerializedHashSetInstanceDrawer(SerializedProperty property, FieldInfo fieldInfo)
        {
            _fieldInfo = fieldInfo;
            ListProperty = property.FindPropertyRelative(SerializedHashSetDrawer.SerializedListName);
            ...
            DefaultState = new HashSetDefaultListState(this);
            SearchState = new HashSetSearchListState(this);
            ...
            var listField = _fieldInfo.FieldType.GetField(SerializedHashSetDrawer.SerializedListName, BindingFlags.Instance | BindingFlags.NonPublic);
            var entryType = listField.FieldType.GetGenericArguments()[0];
            _keyFieldInfo = entryType.GetField(SerializedHashSetDrawer.ElementName);
        ...
        public float GetPropertyHeight(GUIContent label)
        {
            if (!ListProperty.isExpanded)
                return SerializedHashSetDrawer.TopHeaderClipHeight;
        ...
        private void DoList(Rect position)
        {
            ...
            {
                using (new GUI.ClipScope(new Rect(0, position.y, position.width + position.x, SerializedHashSetDrawer.TopHeaderClipHeight)))
                {
        ...
        private SerializedProperty GetElementProperty(SerializedProperty property)
        {
        ...
        {
            var propInfo = dictionary.GetType().GetProperty(SerializedHashSetDrawer.LookupTableName, BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        {
            var list = new ReorderableList(SerializedHashSetDrawer.NoEntriesList, typeof(int));
        ...
        {
            _showSearchBar = flag;
            ReorderableList.headerHeight = SerializedHashSetDrawer.TopHeaderClipHeight + SerializedHashSetDrawer.ElementHeaderHeight + (_showSearchBar ? SerializedHashSetDrawer.SearchHeaderHeight : 0);
            if (!_showSearchBar)
            {
        ...
        {
            Rect topRect = rect.WithHeight(SerializedHashSetDrawer.TopHeaderHeight);
            ...
            if (_showSearchBar)
            {
                adjustedTopRect = adjustedTopRect.AppendDown(SerializedHashSetDrawer.SearchHeaderHeight);
                DoSearch(adjustedTopRect);
            }
            DoKeyValueRect(adjustedTopRect.AppendDown(SerializedHashSetDrawer.ElementHeaderHeight));
        ...
        {
            EditorGUI.DrawRect(rect.AppendLeft(1), SerializedHashSetDrawer.BorderColor);
            EditorGUI.DrawRect(rect.AppendRight(1, -1), SerializedHashSetDrawer.BorderColor);
            EditorGUI.DrawRect(rect.AppendDown(1, -1), SerializedHashSetDrawer.BorderColor);
        ...
        private void DoDisplayTypeToggle(Rect contentRect, bool fieldFlag)
        {
            ...
            if (displayData.Settings.HasListDrawerToggle)
            {
                ...
                EditorGUI.BeginChangeCheck();
                bool newValue = GUI.Toggle(rightRectToggle, displayData.IsListToggleActive, SerializedHashSetDrawer.DisplayTypeToggleContent, EditorStyles.toolbarButton);
                if (EditorGUI.EndChangeCheck())
                {
